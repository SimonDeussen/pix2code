...
def run(input_path, output_path, is_memory_intensive=False, pretrained_model=None):
    ...
    model_summary.write(model.model.summary())
    model_summary.write("")
    model_summary.write("CONTEXT_LENGTH " + CONTEXT_LENGTH)
    model_summary.write("IMAGE_SIZE " + IMAGE_SIZE)
    model_summary.write("BATCH_SIZE " + BATCH_SIZE)
    model_summary.write("EPOCHS " + EPOCHS)
    model_summary.write("STEPS_PER_EPOCH " + STEPS_PER_EPOCH)
    model_summary.write("input_shape " + input_shape)
    model_summary.write("output_size " + output_size)
